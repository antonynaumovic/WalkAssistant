...
def acceleration_handler(addr, *message):
    ...
    try:
        logger.debug(f"Notifying message callbacks with {now} {x} {y} {z}")
        _notify_message(
            {
                "time": now,
                "endpoint": addr,
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(cb):
                try:
                    ...
                    loop.create_task(cb(running))
                except RuntimeError:
                    # No running loop, just try to call it (might fail)
                    asyncio.run(cb(running))
            else:
                cb(running)
        except Exception:
    ...
    for cb in list(_ip_callbacks):
        try:
            if asyncio.iscoroutinefunction(cb):
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(cb(ip))
                except RuntimeError:
                    asyncio.run(cb(ip))
            else:
                cb(ip)
        except Exception:
            logger.exception("Error in ip callback")
